...
class Rectangle extends Shape {
    ...
    pointIsInside(x2, y2) {
        ...
        let horizontalMeasure = false; //if (y2 < y1)   // point at left
        if (y2 > y1) { // point at right
            let horizontalMeasure = y2 <= (y1 + this.width)
        }

        let verticalMeasure = false // if (x2 > x1) {  // point at Up
        if (x2 < x1) {    // point at Down
            let verticalMeasure = x2 <= (x1 + this.width)
        }
